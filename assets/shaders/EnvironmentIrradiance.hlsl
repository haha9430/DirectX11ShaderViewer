...
float3 CubeFaceUvToDirection(uint faceIndex, float2 uv)
{
    ...
    if (faceIndex == 0) return normalize(float3(1.0f, -p.y, -p.x));
    if (faceIndex == 1) return normalize(float3(-1.0f, -p.y, p.x));
    if (faceIndex == 2) return normalize(float3(p.x, 1.0f, p.y));
    if (faceIndex == 3) return normalize(float3(p.x, -1.0f, -p.y));
    if (faceIndex == 4) return normalize(float3(p.x, -p.y, 1.0f));
...
float4 PSMain(VSOutput input) : SV_TARGET
{
    ...
    for (uint i = 0; i < SAMPLE_COUNT; ++i)
    {
        ...
        const float3 localDirection = float3(cos(phi) * sinTheta, sin(phi) * sinTheta, cosTheta);
        const float3 worldDirection = normalize(
            tangent * localDirection.x +
            bitangent * localDirection.y +
            normal * localDirection.z);
        irradiance += EnvironmentTexture.SampleLevel(LinearSampler, worldDirection, 0.0f).rgb;
    }
